...
pub fn 
    hero1_simple_movement_system(entities: &Entities,
            ...
            join!(&entities && &leaders && &retreats && &teams && &is_caught)
        {
            ...
            if leader.0 == 1 {
                if caught.0 {
                    ...
                    if stats.get(e).unwrap().stats.get(&Stats::Health).unwrap().value <= flee.0 {
                        for (point, team, _) in join!(&positions && &teams && &cores){
                            if team.unwrap() == leader_team {
                                targets.insert(e, AiDestination::new(point.unwrap().clone())).unwrap();
                            }
                        }
                    } else {
                        for (e, _, pos) in
                            join!(&entities && &simple_movements && &positions)
                        {
                            let e = e.unwrap();
                            let pos = pos.unwrap();
                            // find closest creep
